...
use x::*;
use xcb::{render, shape, x, Connection, VoidCookie, Xid};
...
const TITLE: &[u8] = b"Ruler";
const CLASS: &[u8] = b"Ruler\0Ruler\0";
...
) -> Result<(), Box<VersionMismatchError>> {
...
impl XCBObjects {
    fn setup(width: u16, height: u16) -> Result<XCBObjects, Box<dyn Error>> {
        ...
        check_versions(
            render::MAJOR_VERSION,
            render::MINOR_VERSION,
            reply.major_version(),
            reply.minor_version(),
            render::XNAME,
        )?;
        ...
        xcb.conn.send_and_check_request(&ChangeProperty {
            ...
            r#type: ATOM_ATOM,
            data: &[
                xcb.atoms.net_wm_state_above,
                xcb.atoms.new_wm_state_skip_pager,
            ],
        })?;
        ...
        xcb.conn.send_and_check_request(&CreateGc {
            ...
            value_list: &[
                Gc::Background(xcb.screen.black_pixel()),
                Gc::GraphicsExposures(false),
            ],
        })?;

        xcb.conn
            .send_and_check_request(&MapWindow { window: xcb.window })?;
...
impl Render {
    fn setup(xcb: &XCBObjects, height: u16, width: u16) -> Result<Render, Box<dyn Error>> {
        let surface = unsafe {
            let cairo_conn = XCBConnection::from_raw_none(
                xcb.conn.get_raw_conn() as *mut cairo::ffi::xcb_connection_t
            );
            let visual_type = XCBVisualType::from_raw_none(
                &xcb.visual_type as *const Visualtype as *mut cairo::ffi::xcb_visualtype_t,
            );
...
fn main() -> Result<(), Box<dyn Error>> {
    let xcb = Rc::new(XCBObjects::setup(
        (INITIAL_LENGTH + RULER_HALF_WIDTH * 2.0) as u16,
        (RULER_HALF_WIDTH * 2.0) as u16,
    )?);
    ...
    let (mut from, mut to) = {
        ...
        (
            DVec2::new(from_x, from_y),
            DVec2::new(from_x + INITIAL_LENGTH, from_y),
        )
    };
    ...
    loop {
        ...
        match event {
            ...
            xcb::Event::X(Event::MotionNotify(ev)) => match dragging {
                Dragging::From => {
                    ...
                    handle_drag(
                        &mut from,
                        to,
                        DVec2::new(ev.root_x() as f64, ev.root_y() as f64),
                        screen_size,
                        fix_distance,
                        fix_angle,
                    );
            ...
            xcb::Event::X(Event::ButtonRelease(ev)) => {
                if ev.detail() == 1 {
                    ...
                    let pos = update(&xcb, &render, from, to, &mut last_update, true)
                        .unwrap()
                        .pos()
                        .as_dvec2();
...
fn update(
    ...
    if force || now - *last_update > Duration::from_millis(16) {
        ...
        render.resize(geometry.w as i32, geometry.h as i32).ok()?;
...
fn handle_drag(
    ...
    if fix_distance {
        let new_diff_normalized = (new_vec - other)
            .try_normalize()
        ...
        new_vec =
            closest_point_below_line_on_circle(other, old_distance, DVec2::ZERO, DVec2::X, new_vec);
        new_vec =
            closest_point_below_line_on_circle(other, old_distance, screen_size, DVec2::X, new_vec);
        new_vec =
            closest_point_below_line_on_circle(other, old_distance, DVec2::ZERO, DVec2::Y, new_vec);
        new_vec =
            closest_point_below_line_on_circle(other, old_distance, screen_size, DVec2::Y, new_vec);
    }
...
fn draw(ctx: &Context, from: DVec2, to: DVec2) -> Result<(), Box<dyn Error>> {
    ...
    for i in (0..length_pixels).step_by(5) {
        let inner_width = RULER_HALF_WIDTH
            - match i % 50 {
                0 => 17.0,
                25 => 12.0,
                _ => 7.0,
    ...
    let display_angle = if angle > 0.0 {
        PI * 2.0 - angle
    } else {
        angle.abs()
    } * 180.0
